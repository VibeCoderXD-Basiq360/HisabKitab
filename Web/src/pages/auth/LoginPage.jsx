...
function StyledInput({ label, focusRef, ...props }) {
    ...
    <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: 24, background: '#F0F2F7' }}>
      <div style={{ width: '100%', maxWidth: 380 }}>
        ...
        <div style={{ textAlign: 'center', marginBottom: 28 }}>
          ...
          <p style={{ fontSize: 14, color: '#B0B8C4', margin: 0 }}>{t('auth.tagline_login')}</p>
        </div>

        {/* Card */}
        <div style={{ background: '#fff', borderRadius: 22, padding: 28, boxShadow: '0 4px 24px rgba(0,0,0,0.08)' }}>
          <form onSubmit={loginWithEmail} style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            <StyledInput
              label={t('auth.email')}
              type="email"
              value={email}
          ...
          {/* Links */}
          <p style={{ textAlign: 'center', fontSize: 13, color: '#B0B8C4', marginTop: 16, marginBottom: 0 }}>
            <Link to="/forgot-password" style={{ color: '#00C2B2', fontWeight: 600, textDecoration: 'none' }}>
        ...
        <p style={{ textAlign: 'center', fontSize: 14, color: '#B0B8C4', marginTop: 20 }}>
          {t('auth.no_account')}{' '}
          <Link to="/signup" style={{ color: '#00C2B2', fontWeight: 700, textDecoration: 'none' }}>
            {t('auth.signup')}
          </Link>
        </p>
      </div>
