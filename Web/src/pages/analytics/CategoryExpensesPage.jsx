...
export default function CategoryExpensesPage() {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', minHeight: '100vh' }}>
      <TopBar title={category?.name || 'Category'} showBack />
      ...
      <div style={{ flex: 1, padding: '16px', paddingBottom: 'calc(100px + env(safe-area-inset-bottom))', display: 'flex', flexDirection: 'column', gap: 16 }}>
        ...
        <div style={{
          ...
          <div style={{
            ...
            height: 52,
            borderRadius: 16,
            background: category?.color ? `${category.color}30` : 'rgba(255,255,255,0.12)',
            display: 'flex',
            alignItems: 'center',
          ...
          {isLoading ? (
            ...
            <SurfaceCard style={{ padding: 0, overflow: 'hidden' }}>
              {expenses.map((expense, idx) => (
                <TransactionRow
                  key={expense.id}
                  icon={expense.category?.icon || category?.icon || '🏷️'}
                  iconBg={expense.category?.color ? `${expense.category.color}25` : (category?.color ? `${category.color}25` : '#E6FAF9')}
